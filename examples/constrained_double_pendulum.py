...
def final_cost(state: jnp.ndarray) -> float:
    ...
    _wrapped = jnp.hstack(
        (wrap_angle(state[0]), wrap_angle(state[1]), state[2], state[3])
    )
...
def double_pendulum(state: jnp.ndarray, action: jnp.ndarray) -> jnp.ndarray:
    ...
    # gravity
    tau = -g * jnp.array([(m1 + m2) * l1 * s1 + m2 * l2 * s12, m2 * l2 * s12])
...
_, (mpc_x_par, mpc_u_par) = jax.lax.scan(
    _jitted_par_mpc_loop, (x0, u0), xs=None, length=800
)
